...
|`MYSQL_HOST`| Host name or address of the MySQL server. | `localhost` |
|`MYSQL_PORT`| Port the MySQL server is listening on. | `3306` |
|`MYSQL_ROOT_PASSWORD`| MySQL server root password. This is only required for initial setup.|  |
...
```
docker pull cspack77/mythbackend:latest
```
...
container image.
  ...
  3.  In the search bar, type the name of the container (`cspack77/mythbackend`).
...
[create a new issue]: https://github.com/cspack77/docker-mythtv/issues
